...
Invasion.Menu.prototype = {
    preload:function(){
        ...
        this.load.spritesheet("clouds", "assets/images/clouds.png",256,103);
        this.load.spritesheet("powerup", "assets/images/powerup.png", 16, 16);
...
Invasion.Play.prototype = {
    create:function(){
        ...
        this.bg = this.add.tileSprite(0, 0, 200, 272, 'bg');//'rolagem' de fundo
		...
		this.ship.animations.add('up', [2,7], 10, false);
        this.ship.animations.add('down', [2,7], 10, false);
        
        this.clouds = this.add.sprite (this.world.centerX, this.world.centerY-50, "clouds");
        this.clouds.anchor.setTo(.5);
        this.physics.enable(this.clouds);
        ...
        this.time.events.loop(1000 - this.nivel * 100, this.novoInimigo, this);//a cada tempo
        this.time.events.loop(1000 - this.nivel * 100, this.atiraInimigo, this);
    },
    criaUi:function(){
    ...
    atiraInimigo:function(){
        this.inimigos.forEachAlive(function(inimigo){
            if(this.game.rnd.frac()>.5 && inimigo.y<200)
                ...
                this.physics.arcade.moveToObject(bala,this.ship,200);
        },this);
    },
    update:function(){
        ...
        this.physics.arcade.overlap(this.ship, this.powerup, this.pegaPoder, null, this);

        this.physics.arcade.collide(this.ship, this.clouds);
    ...
    eliminaInimigo: function(laser, objeto){
        if(this.inimigos.children.indexOf(objeto) > -1){
            ...
            this.pontos +=10;
            this.contaInimigos++;
            this.chanceBonus = game.rnd.pick([0,1,2,3]);
            if (this.chanceBonus==1){
                this.powerup = this.add.sprite (this.ship.x, this.ship.x, "powerup", 0);
                this.physics.enable(this.powerup);
                this.powerup.body.velocity.y=200;
                this.powerup.animations.add ("powerup", [0,1], 10, true);
    ...
    pegaPoder:function(ship,powerup){
        this.bonus = game.rnd.pick([0,1]);
        if (this.bonus == 0){
            this.vidas++;
            powerup.kill();
            }
    ...
    atira: function(){
        ...
        if(this.time.now > this.lasers.proximo && this.ship.tint == 0x0000ff){
            var laser = this.lasers.getFirstDead();
            if (!laser) return;
            laser.reset(this.ship.x, this.ship.y);
            laser.body.velocity.y = -3000;
            Invasion.laserAud.play();

            this.time.events.add(Phaser.Timer.SECOND * 5, this.tiroDuplo, this);
